...
static void check_id( const char* str, int* id )
{
	int pos = 0;
	char* s = (char*)str;
...
static const char* fmt_str( int* id )
{
...
static void debug_fmt( int* id, int* fmt )
{
...
{
	void replace( char* str, int* src, int* dst, int* af, int* bf )
	{
	...
	void getAlpha( int* id, int* fmt, char* str )
	{
		str[0] = '\0';
	...
	void getTable(int in_a, int out_a, char* str)
	{
		str[0] = '\0';
		if ( in_a < 6 || out_a < 6 )
		{
	...
	int process( const char* from, const char* to, int* s, int* d, int* af, int* bf )
	{
		if (!strs) return 0;

		TRACE_DEBUG( "Processing: '%s >> %s'", from, to );

		if ( only_alpha			&& (!HAS_ALPHA(s)||!HAS_ALPHA(d)))					{ TRACE_DEBUG(" return only_alpha\n"); return 0; }
		if ( s_alpha			&& !HAS_ALPHA(s) )									{ TRACE_DEBUG(" return src_alpha\n"); return 0; }
		if ( d_alpha			&& !HAS_ALPHA(d) )									{ TRACE_DEBUG(" return dst_alpha\n"); return 0; }
		if ( s_alpha_bigger		&& (A(d,bf) > A(s,af)) )							{ TRACE_DEBUG(" return src_alpha_bigger\n"); return 0; }
		if ( d_alpha_bigger		&& (A(s,af) > A(d,bf)) )							{ TRACE_DEBUG(" return dst_alpha_bigger\n"); return 0; }
		if ( same_a_bit			&&((same_a_bit!=A(s,af))||(same_a_bit!=A(d,bf))))	{ TRACE_DEBUG(" return same_a_bit\n"); return 0; }
		if ( same_rbg			&& !SAME_RGB(s,d) )									{ TRACE_DEBUG(" return same_rbg\n"); return 0; }
		if ( no_same_rgb		&& (SAME_R(s,d)||SAME_G(s,d)||SAME_B(s,d)))			{ TRACE_DEBUG(" return no_same_rgb\n"); return 0; }
		if ( no_s_alpha			&& HAS_ALPHA(s) )									{ TRACE_DEBUG(" return no_src_alpha\n"); return 0; }
		if ( no_d_alpha			&& HAS_ALPHA(d) )									{ TRACE_DEBUG(" return no_dst_alpha\n"); return 0; }
...
static void process( const char* from, const char* to, int* s, int* d, int* af, int* bf )
{
	for ( int i = 0; i < ARRAY_SIZE(__conditions); ++i )
	{
		if ( __conditions[i].process( from, to, s, d, af, bf ) )
			return;
	}
...
static void generate_function( const char* from, const char* to )
{
	int src_fmt[4];
	int dst_fmt[4];
	int src_id[6];
...
static void generate_all(void)
{
	...
	for ( int x = 0; x < ARRAY_SIZE(all_formats); ++x )
	{
		printf("// -- %s ---------------------------------------------------------------------------------------------\n", all_formats[x] );

		for ( int y = 0; y < ARRAY_SIZE(all_formats); ++y, ++count )
		{
